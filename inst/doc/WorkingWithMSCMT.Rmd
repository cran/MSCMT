...
By default, `genoud` from package `rgenoud` is used as the optimizer for the (outer) optimization. 
`MSCMT` supports other (outer) optimizers, one of which is `DEoptim` of package `DEoptim`, another evolutionary optimizer.
Although using the default `genoud` as outer optimizer is recommended, we use `DEoptim` in the vignettes for speed reasons.
...
Object `res` now contains single SCM estimations for each unit as well as aggregated information concerning original data, synthesized data, and gaps for all units.
...
Many features and options of package `MSCMT` remained untouched, because this would have lead far beyond the scope of this simple example. 
